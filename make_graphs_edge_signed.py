...
output_dir    = "/vols/sbn/uboone/ll4420/dark_tridents_wspace/DM-GNN/graphs_edge_signed/"
N_NEIGHBORS   = 8
BATCH_SIZE    = 5000
...
def process_single_event(args):
    ...
    # Signed edge features: wire diff, time diff, ADC diff (NO abs)
    wire_diff = (sp[src, 0] - sp[dst, 0]).reshape(-1, 1)
    time_diff = (sp[src, 1] - sp[dst, 1]).reshape(-1, 1)
    adc_diff  = (sp[src, 2] - sp[dst, 2]).reshape(-1, 1)
    edge_attr = np.hstack([wire_diff, time_diff, adc_diff])
...
def process_and_save_hdf5(hdf5_file, prefix):
    ...
    with h5py.File(hdf5_file, 'r') as f:
        event_ids    = sorted(f.keys(), key=lambda x: int(x.split('_')[1]))
        ...
        for i in range(0, total_events, BATCH_SIZE):
            ...
            print(f"[{prefix}] Processed {min(i+BATCH_SIZE, total_events)}/{total_events}, graphs: {len(buffer)}")
...
if __name__ == '__main__':
    # process_and_save_hdf5(training_file, "train")  # already done
